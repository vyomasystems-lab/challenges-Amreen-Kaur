...
async def test2_mux(dut):
    ...
    await Timer(2, units='ns')
    assert dut.out.value == 0b01, "Mux result is incorrect for select input :{A} & input :{B} as the output is not :{C} rather it is 00.".format(A = int(dut.sel.value),B = int(dut.inp12.value),C = int(dut.inp12.value))
    print("Mux result is correct for select input :{A} & input :{B} as the output is :{C} and not 00.".format(A = int(dut.sel.value),B = int(dut.inp12.value),C = int(dut.out.value)))
